#!/bin/bash
# Leaf4 init.sh - full VTEP setup with MLAG bond
# Run before FRR starts

# ── Teardown (safe re-run) ────────────────
ip link set bond0.30 nomaster 2>/dev/null || true
ip link set bond0.40 nomaster 2>/dev/null || true
ip link set eth3 nomaster 2>/dev/null || true
ip link delete bond0.30 2>/dev/null || true
ip link delete bond0.40 2>/dev/null || true
ip link set bond0 down 2>/dev/null || true
ip link delete bond0 2>/dev/null || true

# ── Loopback ──────────────────────────────
ip addr add 10.255.1.4/32 dev lo 2>/dev/null || true
ip link set lo up

# ── Underlay interfaces ───────────────────
ip addr add 10.0.4.2/30 dev eth0 2>/dev/null || true
ip addr add 10.0.8.2/30 dev eth1 2>/dev/null || true
ip addr add 10.0.21.2/30 dev eth2 2>/dev/null || true
ip link set eth0 up
ip link set eth1 up
ip link set eth2 up
ip link set eth3 up
...
# ── L2VNI 10010 ────────────────────────────
ip link add vxlan10010 type vxlan id 10010 dstport 4789 local 10.255.1.4 nolearning 2>/dev/null || true
...
# ── L2VNI 10020 ────────────────────────────
ip link add vxlan10020 type vxlan id 10020 dstport 4789 local 10.255.1.4 nolearning 2>/dev/null || true
...
# ── L2VNI 10030 ────────────────────────────
ip link add vxlan10030 type vxlan id 10030 dstport 4789 local 10.255.1.4 nolearning 2>/dev/null || true
...
# ── L2VNI 10040 ────────────────────────────
ip link add vxlan10040 type vxlan id 10040 dstport 4789 local 10.255.1.4 nolearning 2>/dev/null || true
...
# ── L3VNI 50010 ────────────────────────────
ip link add vxlan50010 type vxlan id 50010 dstport 4789 local 10.255.1.4 nolearning 2>/dev/null || true
...
# ── L3VNI 50020 ────────────────────────────
ip link add vxlan50020 type vxlan id 50020 dstport 4789 local 10.255.1.4 nolearning 2>/dev/null || true
...
ip link set eth3 up
echo 44:38:39:FF:02:02 > /sys/class/net/bond0/bonding/ad_actor_system

ip link add link bond0 name bond0.30 type vlan id 30 2>/dev/null || true
ip link set bond0.30 master br30
ip link set bond0.30 up

ip link add link bond0 name bond0.40 type vlan id 40 2>/dev/null || true
ip link set bond0.40 master br40
ip link set bond0.40 up

ip link set br30 type bridge vlan_filtering 1
ip link set br40 type bridge vlan_filtering 1
bridge vlan add dev bond0.30 vid 30 pvid untagged
bridge vlan del dev bond0.30 vid 1
bridge vlan add dev bond0.40 vid 40 pvid untagged
bridge vlan del dev bond0.40 vid 1
bridge vlan add dev br30 vid 30 self
bridge vlan add dev br40 vid 40 self
bridge vlan add dev vxlan10030 vid 30
bridge vlan add dev vxlan10040 vid 40
