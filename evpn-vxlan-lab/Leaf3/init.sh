#!/bin/bash
# Leaf3 init.sh - full VTEP setup with MLAG bond
...
# ── Loopback ──────────────────────────────
ip addr add 10.255.1.3/32 dev lo 2>/dev/null || true
ip link set lo up

# ── Underlay interfaces ───────────────────
ip addr add 10.0.3.2/30 dev eth0 2>/dev/null || true
ip addr add 10.0.7.2/30 dev eth1 2>/dev/null || true
ip addr add 10.0.21.1/30 dev eth2 2>/dev/null || true
ip link set eth0 up
ip link set eth1 up
ip link set eth2 up
ip link set eth3 up
...
ip link add vxlan10010 type vxlan id 10010 dstport 4789 local 10.255.1.3 nolearning 2>/dev/null || true
...
ip link add vxlan10020 type vxlan id 10020 dstport 4789 local 10.255.1.3 nolearning 2>/dev/null || true
...
ip link add vxlan10030 type vxlan id 10030 dstport 4789 local 10.255.1.3 nolearning 2>/dev/null || true
...
ip link add vxlan10040 type vxlan id 10040 dstport 4789 local 10.255.1.3 nolearning 2>/dev/null || true
...
ip link add vxlan50010 type vxlan id 50010 dstport 4789 local 10.255.1.3 nolearning 2>/dev/null || true
...
ip link add vxlan50020 type vxlan id 50020 dstport 4789 local 10.255.1.3 nolearning 2>/dev/null || true
