#!/bin/bash
# Leaf1 init.sh - full VTEP setup with MLAG bond
...
# ── Loopback ──────────────────────────────
ip addr add 10.255.1.1/32 dev lo 2>/dev/null || true
ip link set lo up

# ── Underlay interfaces ───────────────────
ip addr add 10.0.1.2/30 dev eth0 2>/dev/null || true
ip addr add 10.0.5.2/30 dev eth1 2>/dev/null || true
ip addr add 10.0.20.1/30 dev eth2 2>/dev/null || true
ip link set eth0 up
ip link set eth1 up
ip link set eth2 up
ip link set eth3 up
...
ip link add vxlan10010 type vxlan id 10010 dstport 4789 local 10.255.1.1 nolearning 2>/dev/null || true
...
ip link add vxlan10020 type vxlan id 10020 dstport 4789 local 10.255.1.1 nolearning 2>/dev/null || true
...
ip link add vxlan10030 type vxlan id 10030 dstport 4789 local 10.255.1.1 nolearning 2>/dev/null || true
...
ip link add vxlan10040 type vxlan id 10040 dstport 4789 local 10.255.1.1 nolearning 2>/dev/null || true
...
ip link add vxlan50010 type vxlan id 50010 dstport 4789 local 10.255.1.1 nolearning 2>/dev/null || true
...
ip link add vxlan50020 type vxlan id 50020 dstport 4789 local 10.255.1.1 nolearning 2>/dev/null || true
...
# ── eth3 trunk uplink to SW1 ──────────────
# eth3 carries tagged VLANs from SW1
# Each VLAN gets forwarded to its respective bridge
ip link set eth3 up
# VLAN subinterfaces on eth3 into respective bridges
ip link add link eth3 name eth3.10 type vlan id 10 2>/dev/null || true
ip link set eth3.10 master br10
ip link set eth3.10 up
ip link add link eth3 name eth3.20 type vlan id 20 2>/dev/null || true
...
# Set shared MLAG system MAC (must match Leaf2)
echo 44:38:39:FF:01:01 > /sys/class/net/bond0/bonding/ad_actor_system

# Bond VLAN subinterfaces into bridges
...
# Add VLANs to bridge and vxlan interfaces
